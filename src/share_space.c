...
void sharespace_reinit(void)
{
    while(1)
    {
        memcpy((void *)&arm_head, (void *)(dts_sharespace.arm_write_addr + SHARE_SPACE_HEAD_OFFSET), sizeof(struct msg_head_t));
        ...
        else if (arm_head.init_state == 2 && arm_head.write_addr != 0Xa5a5a5a5 && arm_head.read_addr != 0Xa5a5a5a5)
        {
            ...
            memcpy((void *)(dts_sharespace.arm_write_addr + SHARE_SPACE_HEAD_OFFSET ), &arm_head, sizeof(struct msg_head_t));
            break;
        }
...
void share_space_clear(void)
{
    ...
    memset((void *)(dts_sharespace.arm_write_addr + SHARE_SPACE_HEAD_OFFSET  ),0xa5, sizeof(struct msg_head_t));
    printf("share_space_clear init_state  %x\n",0XA5 );
}
...
//    GAM_DEBUG_printf("dsp_head.read_addr",dsp_head.read_addr);
    ...
    memcpy((void *)(mmap_sharespace.dsp_write_addr + SHARE_SPACE_HEAD_OFFSET ), &dsp_head, sizeof(struct msg_head_t));
        ...
        if((state_p->arm_wr_addr_dsp_cmd) == DSP_READ_STATE )
        {
             if((state_p->arm_wr_addr_arm_cmd) == SYS_IDLE_STATE )
             {
                ...
                return 1;
                    ...
                    while((state_p->arm_wr_addr_dsp_cmd) != SYS_IDLE_STATE )
                    {
                        printf("share_space_SDA %d: %x %x %x\n",__LINE__,state_p->arm_wr_addr_state ,state_p->arm_wr_addr_dsp_cmd,state_p->arm_wr_addr_arm_cmd );
                        state_p->arm_wr_addr_dsp_cmd = SYS_IDLE_STATE;
                    }
                    return 0;
             }
...
int do_read_space(uint8_t* buf, uint32_t size)
{
    ...
    {
            printf("share_space_read wait msg  %d %d\n",msgbox_new_msg[0],msgbox_new_msg[1] );
    }
#endif
   #if  MSGBOX_EN_SYNC
